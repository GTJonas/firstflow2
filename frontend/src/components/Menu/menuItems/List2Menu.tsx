...
const List2Menu = () => {
    ...
    const handleRedirect = async () => {
        try {
            const response = await axios.get("http://192.168.1.78:8000/api/company/own-profile", {
    ...
    if (companyUuid !== null) {
        list2.push({ label: "Företagsprofilen", link: `/company/${companyUuid}`, icon: "icon-settings" });
    }
