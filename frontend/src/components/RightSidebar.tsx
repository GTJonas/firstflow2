...
function RightSidebar({ user }) {
    ...
    async function fetchData() {
        try {
            const response = await axios.get('http://192.168.1.78:8000/api/show-own-class', {
            ...
            case 2: // Supervisor
                return (
                    <>
                        ...
                        {classes.length > 0 ? (
                            <ul>
                                {classes.flatMap(classItem => classItem.teacher).map(teacher => (
                                    <li key={teacher.uuid}>
                                        <img
                                            ...
                                            alt="Teacher Profile"
                                        />
                                        <p>{teacher.first_name} {teacher.last_name} - {teacher.classname}</p>
                                    </li>
                                ))}
                            </ul>
                        ) : (
                                ...
                                {classes.flatMap(classItem => classItem.students).map(student => (
                                    <li key={student.uuid}>
                                        <img
                                            ...
                                            alt="Student Profile"
                                        />
                                        <p>{student.first_name} {student.last_name} - {student.classname}</p>
                                    </li>
                                ))}
                            </ul>
                        ) : (
                                    ...
                                    <li key={classItem.classid}>
                                        ...
                                        <p>
                                            <img
                                                ...
                                                alt="Teacher Profile"
                                            />
                                            {classItem.teacher.first_name} {classItem.teacher.last_name} - Lärare
                                        </p>
                                        <ul>
                                            {classItem.students.map(student => (
                                                <li key={student.uuid}>
