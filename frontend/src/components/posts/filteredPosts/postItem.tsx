...
const PostItem = ({ post, user }) => {
    ...
    const handleAccept = async () => {
        try {
            const response = await axios.post(`${API_BASE_URL}/posts/${post.uuid}/accept`, null, { headers });
    ...
    const handleDecline = async () => {
        try {
            const response = await axios.post(`${API_BASE_URL}/posts/${post.uuid}/decline`, null, { headers });
            if (response.status === 200) {
                // Update the status in the post object
...
// Utility function to map enum values to human-readable names
    const mapStatusToName = (status) => {
        switch (status) {
            case 'approved':
                return <><i className="icon-approved"></i><p>Godkänd</p></>;
            case 'rejected':
                return <><i>!</i><p>Nekad</p></>;
            default:
                return <><i>!</i><p>Inte behandlad</p></>;
        }
    };
    ...
    return (
        <li key={post.uuid} className="post rounded">
            ...
            <p>{formatTime(post.from)} - {formatTime(post.to)}</p>
