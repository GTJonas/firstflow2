...
const AssignStudentsToClass = () => {
    ...
    useEffect(() => {
        async function fetchClassesAndStudents() {
            try {
                const responseClasses = await axios.get('http://192.168.1.78:8000/api/show-own-class', {
                    headers: {
                        ...getAuthHeaders(),
                    },
                });
    ...
    const handleDetachStudents = async () => {
        try {
            const responseDetach = await axios.put(
                'http://192.168.1.78:8000/api/detach-students-from-class',
                {
                    class_id: classId,
                    student_ids: studentIds,
                },
            ...
            setStudents(students.filter(student => !studentIds.includes(student.uuid))); // Make sure to adjust this to match your data structure
        } catch (error) {
            console.error(error);
            ...
            <div>
                ...
                >
                    {students.map((student) => {
                        if (student.role_id === 4) { // Adjusted condition here
                            return (
