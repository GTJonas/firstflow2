...
const ShowOwnSchoolClass = () => {
        ...
        async function fetchData() {
            try {
                const [classDetailsResponse, companiesResponse] = await Promise.all([
                    axios.get('http://192.168.1.78:8000/api/show-own-class', {headers: getAuthHeaders()}),
                    axios.get('http://192.168.1.78:8000/api/company/all/show', {headers: getAuthHeaders()})
                ]);
                ...
                if (classDetailsResponse.data.classes.length > 0) {
                    const schoolId = classDetailsResponse.data.classes[0].teacher.school_id;

                    const responseStudents = await axios.get(`http://192.168.1.78:8000/api/get-all-students-from-school/${schoolId}`, {
                        headers: {
                            ...getAuthHeaders(),
                        },
                    });

                    const studentsFromSchool = responseStudents.data.filter(student => student.role_id === 4);
    ...
    const updateUserCompany = async (classIndex, studentIndex, newCompanyUuid) => {
        try {
            const response = await axios.put(
                'http://192.168.1.78:8000/api/change-company',
                {
    ...
    const handleAssignStudents = async () => {
        try {
            const response = await axios.put('http://192.168.1.78:8000/api/assign-students-to-class', {
                class_id: classId,
    ...
    const handleDetachStudent = async (studentUuid) => {
        try {
            ...
            setStudents(prevStudents => prevStudents.filter(student => student.uuid !== studentUuid));
    ...
    return (
        <div>
            {Array.isArray(classDetailsList) && classDetailsList.map((classDetails, classIndex) => (
                <div key={classIndex}>
                    <h2>{classDetails.classname}</h2>
                    <p>Teacher: {classDetails.teacher.first_name} {classDetails.teacher.last_name}</p>
                    {isAssignDetachMode ? (
                        <div>
                                ...
                                {classDetails.students.map((student, studentIndex) => (
                                    <li key={studentIndex}>
                                        ...
                                        {isAssignDetachMode && (
                                            <select
                                                className="ms-2"
                                                value={selectedCompanies[classIndex]?.[studentIndex] || ''}
                                                onChange={(event) => handleCompanyChange(event, classIndex, studentIndex)}
                                            >
            ...
            <button onClick={() => setIsAssignDetachMode(true)}>Switch to Assign/Detach Mode</button>
            <button onClick={() => setIsAssignDetachMode(false)}>Switch to View Mode</button>
